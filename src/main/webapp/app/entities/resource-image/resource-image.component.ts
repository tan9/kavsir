...
export class ResourceImageComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.resourceImageService
        ...
        .subscribe(
          (res: HttpResponse<IResourceImage[]>) => this.paginateResourceImages(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/resource-image'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IResourceImage) {
    return item.id;
  }
  ...
  protected paginateResourceImages(data: IResourceImage[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.resourceImages = data;
  }
