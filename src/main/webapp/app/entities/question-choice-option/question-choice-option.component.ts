...
export class QuestionChoiceOptionComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.questionChoiceOptionService
        ...
        .subscribe(
          (res: HttpResponse<IQuestionChoiceOption[]>) => this.paginateQuestionChoiceOptions(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/question-choice-option'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: IQuestionChoiceOption) {
    return item.id;
  }
  ...
  protected paginateQuestionChoiceOptions(data: IQuestionChoiceOption[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    this.questionChoiceOptions = data;
  }
