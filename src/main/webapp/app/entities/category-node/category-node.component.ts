...
export class CategoryNodeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    if (this.currentSearch) {
      this.categoryNodeService
        ...
        .subscribe(
          (res: HttpResponse<ICategoryNode[]>) => this.paginateCategoryNodes(res.body, res.headers),
  ...
  transition() {
    this.router.navigate(['/category-node'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  trackId(index: number, item: ICategoryNode) {
    return item.id;
  }
  ...
  protected paginateCategoryNodes(data: ICategoryNode[], headers: HttpHeaders) {
